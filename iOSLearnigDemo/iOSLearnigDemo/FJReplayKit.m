        ...
        if ([[RPScreenRecorder sharedRecorder] isAvailable]) {
            ...
            [[RPScreenRecorder sharedRecorder] startRecordingWithMicrophoneEnabled:YES handler:^(NSError *error) {
                if (error) {
                    ...
                    if ([_delegate respondsToSelector:@selector(replayRecordFinishWithVC:errorInfo:)]) {[_delegate replayRecordFinishWithVC:nil errorInfo:[NSString stringWithFormat:@"FJReplayKit:开始录制error %@", error]];}
                }
                else {
                    NSLog(@"FJReplayKit:开始录制");
                    [self.startBt setTitle:@"正在录制" forState:UIControlStateNormal];
                    if ([_delegate respondsToSelector:@selector(replayRecordStart)]) {[_delegate replayRecordStart];}
                }
            }];
        }
        else {
            NSLog(@"FJReplayKit:环境不支持ReplayKit录制");
            if ([_delegate respondsToSelector:@selector(replayRecordFinishWithVC:errorInfo:)]) {[_delegate replayRecordFinishWithVC:nil errorInfo:@"FJReplayKit:环境不支持ReplayKit录制"];}
        }
    }
    else {
        NSLog(@"FJReplayKit:系统版本需要是iOS9.0及以上才支持ReplayKit录制");
        if ([_delegate respondsToSelector:@selector(replayRecordFinishWithVC:errorInfo:)]) {[_delegate replayRecordFinishWithVC:nil errorInfo:@"FJReplayKit:系统版本需要是iOS9.0及以上才支持ReplayKit录制"];}
    }
} //结束录制
- (void)stopRecordAndShowVideoPreviewController:(BOOL)isShow {
    ...
    [[RPScreenRecorder sharedRecorder] stopRecordingWithHandler:^(RPPreviewViewController *previewViewController, NSError *error) {
        ...
        if (error) {
            ...
            if ([_delegate respondsToSelector:@selector(replayRecordFinishWithVC:errorInfo:)]) {[_delegate replayRecordFinishWithVC:nil errorInfo:[NSString stringWithFormat:@"FJReplayKit:结束录制error %@", error]];}
        }
        else {
            NSLog(@"FJReplayKit:录制完成");
            if ([_delegate respondsToSelector:@selector(replayRecordFinishWithVC:errorInfo:)]) {[_delegate replayRecordFinishWithVC:previewViewController errorInfo:@""];}
            if (isShow) {[self showVideoPreviewController:previewViewController animation:YES];}
        }
    }];
}

#pragma mark - 显示/关闭视频预览页 //显示视频预览页面
...
- (void)showVideoPreviewController:(RPPreviewViewController *)previewController animation:(BOOL)animation {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        else {previewController.view.frame = rect;}
...
- (void)previewController:(RPPreviewViewController *)previewController didFinishWithActivityTypes:(NSSet <NSString *> *)activityTypes {
    if ([activityTypes containsObject:@"com.apple.UIKit.activity.SaveToCameraRoll"]) {
        ...
        if ([_delegate respondsToSelector:@selector(saveSuccess)]) {[_delegate saveSuccess];}
    }
    if ([activityTypes containsObject:@"com.apple.UIKit.activity.CopyToPasteboard"]) {NSLog(@"FJReplayKit:复制成功");}
}

#pragma mark - 其他方法 //判断对应系统版本是否支持ReplayKit

- (BOOL)systemVersionOK {
    if ([[UIDevice currentDevice].systemVersion floatValue] < 9.0) {return NO;} else {return YES;}
} //获取rootVC
